...
See videos below for how to employ COMBImageDL with the test data from the exhaustive *in vitro* evaluation of the CUSP9 protocol and reproduce the corresponding results. In case you want to run COMBImageDL with newly generated data, you have first to segment the corresponding label-free live-cell imaging data with the the slighlty modified version of U-Net that COMBImage is using. The trained network as well as all required MATLAB scripts are provided [./UNet Segmentation](UNet).
